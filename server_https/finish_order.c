...
void finish_order_cb(struct evhttp_request *req, void *arg){
    ...
    //得到订单信息
    ret = curl_to_cacheserver_get_order(orderid->valuestring, passenger_sessionid, driver_sessionid, order_info);

    if(ret == 0){
        //根据passenger_sessionid得到乘客用户名
        ret = curl_to_cacheserver_get_username(passenger_sessionid, order_info->passenger_username);
    }

    if(ret == 0){
        //根据driver_sessionid得到四级用户名
    ...
    if(ret == 0){
        //将 司机的 sessionID中的orderid设置为 NONE 
